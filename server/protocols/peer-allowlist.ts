/**
 * Peer allowlist shared by the unauthenticated-protocol TCP listeners.
 *
 * Originally written for Modbus TCP Server Mode (#462) and moved here verbatim
 * when the DNP3 outstation listener (#464) needed exactly the same control. Both
 * protocols have NO authentication in the protocol itself — no credential, no
 * handshake, no integrity protection on the wire. Anything that can open a TCP
 * connection to such a listener is, protocol-wise, a fully privileged master.
 * The only place 0xSCADA can compensate is the deployment boundary, so those
 * listeners refuse connections from any peer that is not explicitly allowlisted
 * by IP or CIDR, checked at accept time before a single byte is read.
 *
 * This module is pure (no sockets, no I/O) so every parsing and matching rule is
 * unit-testable. IPv4, IPv6, and IPv4-mapped IPv6 (`::ffff:10.0.0.1`, which is
 * what a dual-stack listener reports for an IPv4 client) all normalise to the
 ...
 * every peer on the internet, which is not an allowlist. The protocols using
 * this module cannot authenticate the peers such a rule would admit, so it fails
 * closed here rather than silently degrading to "anyone may write to the plant".
 *
...
export function parsePeerRule(entry: string): PeerRule {
  ...
  if (prefix === 0) {
    throw new PeerRuleError(
      `"${source}" allows every peer. This listener serves a protocol with no ` +
        "client authentication, so a wildcard allowlist is refused; list the " +
        "specific master IPs or subnets.",
    );
  }

...
if ($DryRun) {
    Write-Host "[DRY-RUN] gh issue create --title '[Optix/Foundation] Implement NetLogic CheatSheet patterns for 0xSCADA' --label 'optix,foundation,netlogic,P0' --body '$issueBody'"
} else {
    Invoke-Expression "gh issue create --title '[Optix/Foundation] Implement NetLogic CheatSheet patterns for 0xSCADA' --label 'optix,foundation,netlogic,P0' --body '$issueBody'"
}
...
if ($DryRun) {
    Write-Host "[DRY-RUN] gh issue create --title '[Optix/Foundation] Study Runtime NetLogic training for event generation' --label 'optix,foundation,netlogic,runtime,P0' --body '$issueBody'"
} else {
    Invoke-Expression "gh issue create --title '[Optix/Foundation] Study Runtime NetLogic training for event generation' --label 'optix,foundation,netlogic,runtime,P0' --body '$issueBody'"
}
...
if ($DryRun) {
    Write-Host "[DRY-RUN] gh issue create --title '[Optix/Foundation] Study Design-Time NetLogic for code generation tooling' --label 'optix,foundation,netlogic,designtime,P0' --body '$issueBody'"
} else {
    Invoke-Expression "gh issue create --title '[Optix/Foundation] Study Design-Time NetLogic for code generation tooling' --label 'optix,foundation,netlogic,designtime,P0' --body '$issueBody'"
}
...
if ($DryRun) {
    Write-Host "[DRY-RUN] gh issue create --title '[Optix/OPC-UA] Implement arbitrary OPC UA read for gateway driver' --label 'optix,opc-ua,gateway,P0' --body '$issueBody'"
} else {
    Invoke-Expression "gh issue create --title '[Optix/OPC-UA] Implement arbitrary OPC UA read for gateway driver' --label 'optix,opc-ua,gateway,P0' --body '$issueBody'"
}
...
if ($DryRun) {
    Write-Host "[DRY-RUN] gh issue create --title '[Optix/OPC-UA] Implement OPC UA Alarms & Events subscription' --label 'optix,opc-ua,alarms,P0' --body '$issueBody'"
} else {
    Invoke-Expression "gh issue create --title '[Optix/OPC-UA] Implement OPC UA Alarms & Events subscription' --label 'optix,opc-ua,alarms,P0' --body '$issueBody'"
}
...
if ($DryRun) {
    Write-Host "[DRY-RUN] gh issue create --title '[Optix/IoT] Integrate Azure IoT Operations for cloud telemetry' --label 'optix,azure,iot,mqtt,P0' --body '$issueBody'"
} else {
    Invoke-Expression "gh issue create --title '[Optix/IoT] Integrate Azure IoT Operations for cloud telemetry' --label 'optix,azure,iot,mqtt,P0' --body '$issueBody'"
}
...
if ($DryRun) {
    Write-Host "[DRY-RUN] gh issue create --title '[Optix/Database] Implement PostgreSQL connection patterns' --label 'optix,database,postgres,P0' --body '$issueBody'"
} else {
    Invoke-Expression "gh issue create --title '[Optix/Database] Implement PostgreSQL connection patterns' --label 'optix,database,postgres,P0' --body '$issueBody'"
}
...
if ($DryRun) {
    Write-Host "[DRY-RUN] gh issue create --title '[Optix/REST] Implement REST/WebSocket server patterns' --label 'optix,rest,websocket,api,P0' --body '$issueBody'"
} else {
    Invoke-Expression "gh issue create --title '[Optix/REST] Implement REST/WebSocket server patterns' --label 'optix,rest,websocket,api,P0' --body '$issueBody'"
}
...
if ($DryRun) {
    Write-Host "[DRY-RUN] gh issue create --title '[Optix/Alarms] Implement custom alarm management engine' --label 'optix,alarms,events,P0' --body '$issueBody'"
} else {
    Invoke-Expression "gh issue create --title '[Optix/Alarms] Implement custom alarm management engine' --label 'optix,alarms,events,P0' --body '$issueBody'"
}
...
if ($DryRun) {
    Write-Host "[DRY-RUN] gh issue create --title '[Optix/Recipes] Implement CFR21-compliant recipe auditing' --label 'optix,recipes,cfr21,compliance,P0' --body '$issueBody'"
} else {
    Invoke-Expression "gh issue create --title '[Optix/Recipes] Implement CFR21-compliant recipe auditing' --label 'optix,recipes,cfr21,compliance,P0' --body '$issueBody'"
}
...
if ($DryRun) {
    Write-Host "[DRY-RUN] gh issue create --title '[Optix/Security] Implement two-factor authentication' --label 'optix,security,2fa,authentication,P0' --body '$issueBody'"
} else {
    Invoke-Expression "gh issue create --title '[Optix/Security] Implement two-factor authentication' --label 'optix,security,2fa,authentication,P0' --body '$issueBody'"
}
...
if ($DryRun) {
    Write-Host "[DRY-RUN] gh issue create --title '[Optix/Observability] Implement Prometheus metrics exporter' --label 'optix,observability,prometheus,metrics,P0' --body '$issueBody'"
} else {
    Invoke-Expression "gh issue create --title '[Optix/Observability] Implement Prometheus metrics exporter' --label 'optix,observability,prometheus,metrics,P0' --body '$issueBody'"
}
...
if ($DryRun) {
    Write-Host "[DRY-RUN] gh issue create --title '[Optix/Edge] Integrate Docker container deployment patterns' --label 'optix,docker,edge,deployment,P0' --body '$issueBody'"
} else {
    Invoke-Expression "gh issue create --title '[Optix/Edge] Integrate Docker container deployment patterns' --label 'optix,docker,edge,deployment,P0' --body '$issueBody'"
}
...
if ($DryRun) {
    Write-Host "[DRY-RUN] gh issue create --title '[Optix/Edge] Study OptixEdge module source for edge integration' --label 'optix,edge,hardware,module,P0' --body '$issueBody'"
} else {
    Invoke-Expression "gh issue create --title '[Optix/Edge] Study OptixEdge module source for edge integration' --label 'optix,edge,hardware,module,P0' --body '$issueBody'"
}
